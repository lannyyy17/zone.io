...
const prompt = ai.definePrompt({
  ...
  prompt: `You are a network analysis expert. Your task is to analyze a session of collected WiFi signal strength data and provide a concise, insightful summary in markdown format.
...
2.  **Key Observations:** In a bulleted list, highlight 2-3 key observations. These could include:
...
3.  **Recommendation (Optional):** If there are clear problem areas, provide a brief, actionable recommendation. For example, "Consider placing a WiFi extender near the area around (lat, lon) to improve coverage."
