...
const prompt = ai.definePrompt({
  ...
  prompt: `You are a network analysis expert. Your task is to analyze a session of collected WiFi signal strength data and provide a concise, insightful summary.
...
1.  **Overall Assessment:** Start with a brief, one-sentence conclusion about the overall signal quality during the session (e.g., "Overall, the signal quality during this session was excellent, with only minor fluctuations.").
2.  **Key Observations:** In an unordered list, highlight 2-3 key observations. These could include:
...
3.  **Recommendation (Optional):** If there are clear problem areas, provide a brief, actionable recommendation with a <strong>Recommendation:</strong> prefix. For example, "<strong>Recommendation:</strong> Consider placing a WiFi extender near the area around (lat, lon) to improve coverage."
...
Generate the summary now. Return the entire summary as a single HTML block.`,
});
